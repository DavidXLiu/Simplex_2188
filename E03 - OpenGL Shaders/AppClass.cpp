...
void AppClass::ProcessKeyboard(sf::Event a_event)
{
	...
	else if (sf::Keyboard::isKeyPressed(sf::Keyboard::Space)) // Enable/disable complimentary colors when space is pressed
...
void AppClass::Display(void)
{
	...
	else if (m_v3Color == glm::vec3(-1.0f, -1.0f, -1.0f)) // Exception for complimentary colors
	{
		// Create complimentary colors
		std::vector<glm::vec3> lVertex;
		//vertex 1
		lVertex.push_back(glm::vec3(-1.0f, -1.0f, 0.0f)); //position
		...
		glUniform3f(SolidColor, glm::abs(m_v3Color.r - 1), glm::abs(m_v3Color.g - 1), glm::abs(m_v3Color.b - 1));
